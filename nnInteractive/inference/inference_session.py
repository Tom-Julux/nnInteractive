...
class nnInteractiveInferenceSession:
    def __init__(
        ...
        self.pad_mode_data = self.preferred_scribble_thickness = (
            self.point_interaction
        ) = None
    ...
    def _is_official_checkpoint(plans: dict, checkpoint: dict) -> bool:
        return (
            ...
            and checkpoint.get("init_args", {}).get("configuration")
    ...
    def set_image(self, image: np.ndarray, image_properties: dict = None):
        ...
        assert (
            image.ndim == 4
        ), f"expected a 4d image as input, got {image.ndim}d. Shape {image.shape}"
        if self.verbose:
    ...
    def set_do_autozoom(
        self, do_propagation: bool, max_num_patches: Optional[int] = None
    ):
    ...
    def _capture_prediction_state(self):
        if self.interactions is None or self.target_buffer is None:
            if self.verbose:
                print(
                    "Skipping prediction state capture because interactions or target_buffer is not initialized."
                )
            return
    ...
    def _restore_prediction_state(self, state: dict):
        if self.interactions is None or self.target_buffer is None:
            raise RuntimeError(
                "Cannot restore state without interactions and target_buffer"
            )
        ...
        if isinstance(self.target_buffer, np.ndarray):
            if not isinstance(target_state, np.ndarray):
                raise RuntimeError(
                    "Stored target buffer type does not match current target buffer type"
                )
            ...
            if target_state.device != self.target_buffer.device:
                target_state = target_state.to(self.target_buffer.device)
            self.target_buffer.copy_(target_state)
        else:
            raise RuntimeError("target_buffer must be np.ndarray or torch.Tensor")
    ...
    def redo_prediction(self) -> bool:
        ...
        try:
            ...
            self._redo_history.append(restored_state)
            raise
    ...
    def _initialize_interactions(self, image_torch: torch.Tensor):
        ...
        use_pinned = (
            not is_linux_kernel_6_11()
            and self.use_pinned_memory
    ...
    def _background_set_image(self, image: np.ndarray, image_properties: dict):
        ...
        self.interactions_future = self.executor.submit(
            self._initialize_interactions, image
        )
    ...
    def add_bbox_interaction(
        self, bbox_coords, include_interaction: bool, run_prediction: bool = True
    ) -> np.ndarray:
        ...
        if self.verbose:
            print(
                f"Added bounding box coordinates.\n"
                f"Raw: {bbox_coords}\n"
        ...
        image_shape = (
            self.preprocessed_image.shape
        )  # Assuming shape is (C, H, W, D) or similar
    ...
    def add_scribble_interaction(
        ...
        assert all(
            [
                i == j
                for i, j in zip(self.original_image_shape[1:], scribble_image.shape)
            ]
        ), f"Given scribble image must match input image shape. Input image was: {self.original_image_shape[1:]}, given: {scribble_image.shape}"
        self._finish_preprocessing_and_initialize_interactions()
        ...
        torch.maximum(
            self.interactions[interaction_channel],
            scribble_image.to(self.interactions.device),
    ...
    def add_lasso_interaction(
        ...
        ), f"Given lasso image must match input image shape. Input image was: {self.original_image_shape[1:]}, given: {lasso_image.shape}"
        self._finish_preprocessing_and_initialize_interactions()
        ...
        torch.maximum(
            self.interactions[interaction_channel],
            lasso_image.to(self.interactions.device),
            out=self.interactions[interaction_channel],
        )
        del lasso_image
        empty_cache(self.device)
    ...
    def add_initial_seg_interaction(
        self, initial_seg: np.ndarray, run_prediction: bool = False
    ):
        """
        ...
        self.interactions[interaction_channel] = initial_seg.to(
            self.interactions.device
        )
        empty_cache(self.device)
        if run_prediction:
    ...
    def _predict(self, force_full_refine: bool = False):
        ...
        ), "pad modes other than constant are not implemented here"
        assert len(self.new_interaction_centers) == len(
            self.new_interaction_zoom_out_factors
        )
        ...
        prediction_center, zoom_out_factor = (
            self.new_interaction_centers[-1],
            self.new_interaction_zoom_out_factors[-1],
        )
        zoom_out_factor = min(4, zoom_out_factor)
        first_prediction_for_history = len(self._undo_history) == 0
        pre_prediction_state = None
        if first_prediction_for_history:
        ...
        with (
            ...
            input_for_predict, scaled_patch_size, scaled_bbox = (
            ...
            if not all([i == j for i, j in zip(pred.shape, previous_prediction.shape)]):
                previous_prediction = interpolate(
                    previous_prediction[None, None].to(float),
                    pred.shape,
                    mode="nearest",
                )[0, 0]
            ...
            while has_change and self.do_autozoom:
                ...
                if not all(
                    [i == j for i, j in zip(pred.shape, previous_prediction.shape)]
                ):
                ...
                has_change = self._detect_change_at_border(
                    pred, previous_prediction_resized
                )

            if zoom_out_factor > 1:
                print(
                    f"Zoom out took {round(time() - start_zoomout, 3)} s, max zoom out factor {zoom_out_factor}"
                )
            else:
            ...
            if zoom_out_factor == 1:
                ...
                bbox = [
                    [i[0] + bbc[0], i[1] + bbc[0]]
                    for i, bbc in zip(
                        scaled_bbox, self.preprocessed_props["bbox_used_for_cropping"]
                    )
                ]
                paste_tensor(
                    self.target_buffer,
        ...
        self.new_interaction_centers = []
        self.new_interaction_zoom_out_factors = []
        if first_prediction_for_history and pre_prediction_state is not None:
            self._undo_history.append(pre_prediction_state)
        self._capture_prediction_state()
        empty_cache(self.device)

    def _build_network_input(self, prediction_center, zoom_out_factor):
        scaled_patch_size = [
            round(i * zoom_out_factor) for i in self.configuration_manager.patch_size
        ]
        scaled_bbox = [
            [c - p // 2, c + p // 2 + p % 2]
        ...
        if not all(
            [
                i == j
                for i, j in zip(
                    self.configuration_manager.patch_size, scaled_patch_size
                )
            ]
        ):
            ...
            if max_pool_ks > 1:
                ...
                for i in range(3, 7):
                    tmp[i : i + 1] = iterative_3x3_same_padding_pool3d(
                        tmp[None, i : i + 1], max_pool_ks
                    )[0]
            crop_interactions_resampled_gpu = interpolate(
                tmp[None], self.configuration_manager.patch_size, mode="area"
            )[0]
            ...
            crop_img = interpolate(
                (
                    pad_cropped(crop_img, pad_image)[None]
                    if any([x for y in pad_interaction for x in y])
                    else crop_img[None]
                ),
            ...
            # crop_img is already on device
            crop_img = (
                pad_cropped(crop_img, pad_image)
                if any([x for y in pad_interaction for x in y])
                else crop_img
            )
            crop_interactions = (
    ...
    def _refine_coarse(self, diff_map, prediction_with_coarse):
        ...
        preallocated_input = torch.zeros(
            (8, *self.configuration_manager.patch_size),
            device=self.device,
            dtype=torch.float,
        )
        for nref, refinement_bbox in enumerate(bboxes_ordered):
            ...
            crop_and_pad_into_buffer(
                preallocated_input[0], refinement_bbox, self.preprocessed_image[0]
            )
            crop_and_pad_into_buffer(
                preallocated_input[1], refinement_bbox, prediction_with_coarse
            )
            crop_and_pad_into_buffer(
                preallocated_input[2:], refinement_bbox, self.interactions[1:]
            )
            ...
            bbox = [
                [i[0] + bbc[0], i[1] + bbc[0]]
                for i, bbc in zip(
                    refinement_bbox, self.preprocessed_props["bbox_used_for_cropping"]
                )
            ]
            paste_tensor(
                self.target_buffer,
    ...
    def _detect_change_at_border(
        ...
        for dim in range(pred.ndim):
            ...
            for idx in [0, pred.shape[dim] - 1]:
                slice_prev = prev_pred.index_select(
                    dim, torch.tensor(idx, device="cpu")
                )
                slice_curr = pred.index_select(
                    dim, torch.tensor(idx, device=self.device)
                ).to("cpu")
                pixels_prev = torch.sum(slice_prev)
                pixels_current = torch.sum(slice_curr)
                pixels_diff = torch.sum(slice_prev != slice_curr)
                rel_change = (
                    max(pixels_prev, pixels_current)
                    / max(min(pixels_prev, pixels_current), 1e-5)
                    - 1
                )
                if pixels_diff > abs_pxl_change_threshold:
                    ...
                    break
                if (
                    pixels_diff > min_pxl_change_threshold
                    and rel_change > rel_pxl_change_threshold
                ):
                    has_change = True
                    if self.verbose:
                        print(
    ...
    def _compute_diff_map(
        self, pred, previous_prediction, scaled_bbox, scaled_patch_size
    ):
        """
        ...
        bbox_tmp = [
            [max(0, i[0]), min(i[1], s)] for i, s in zip(bbox_tmp, scaled_patch_size)
        ]
        ...
        # open distance map
        diff_map[slicer] = iterative_3x3_same_padding_pool3d(
            diff_map[slicer][None, None], kernel_size=5, use_min_pool=True
        )[0, 0]
        diff_map[slicer] = iterative_3x3_same_padding_pool3d(
            diff_map[slicer][None, None], kernel_size=5, use_min_pool=False
        )[0, 0]
    ...
    def _add_patch_for_bbox_interaction(self, bbox):
        ...
        requested_size = [
            i + j // 3 for i, j in zip(bbox_size, self.configuration_manager.patch_size)
        ]
            ...
            max(
                ...
                max(
                    [
                        i / j
                        for i, j in zip(
                            requested_size, self.configuration_manager.patch_size
                        )
                    ]
                ),
            )
    ...
    def _generic_add_patch_from_image(self, image: torch.Tensor):
        ...
        requested_size = [
            i + j // 3 for i, j in zip(roi_size, self.configuration_manager.patch_size)
        ]
        self.new_interaction_zoom_out_factors.append(
            max(
                1,
    ...
    def initialize_from_trained_model_folder(
        ...
        expected_json_file = join(
            model_training_output_dir, "inference_session_class.json"
        )
        ...
        if isinstance(json_content, str):
            ...
            self.preferred_scribble_thickness = json_content[
            ...
            self.interaction_decay = (
                json_content["interaction_decay"]
                if "interaction_decay" in json_content.keys()
                else 0.98
            )
            point_interaction_use_etd = (
                True  # so far this is not defined in that file so we stick with default
            )
            self.point_interaction = PointInteraction_stub(
                point_interaction_radius, point_interaction_use_etd
            )
            ...
            self.pad_mode_data = (
                json_content["pad_mode_image"]
                if "pad_mode_image" in json_content.keys()
                else "constant"
            )
        ...
        if use_fold is not None:
            ...
            assert (
                len(fldrs) == 1
            ), f"Attempted to infer fold but there is != 1 fold_ folders: {fldrs}"
            fold_folder = fldrs[0]
